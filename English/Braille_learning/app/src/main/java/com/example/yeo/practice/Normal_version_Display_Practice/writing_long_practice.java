...
public class writing_long_practice extends FragmentActivity {
    ...
    public void Grading(){
        ...
        if(result==true) {
            ...
            MainActivity.TTS2.speak("That's right!, go to the next question.",TextToSpeech.QUEUE_FLUSH, null);
        }
        else if(result==false){
            m.quiz_target_init();
            MainActivity.TTS2.speak("That’s a wrong answer.Check out the braille and go to the next question.", TextToSpeech.QUEUE_FLUSH, null);
            for(int i=0 ; i<3 ; i++) {
                for (int j = 0; j < 14; j++) {
