...
public class student_practice extends FragmentActivity {
    student_display m;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        MainActivity.TTS2.speak("Please tell me the room number.", TextToSpeech.QUEUE_FLUSH, null);
        timer = new Timer();
    ...
    public void Timer_Start(){ //1초의 딜레이 시간을 갖는 함수
        second = new TimerTask() {
            @Override
            public void run() {
                if(MainActivity.TTS2.isSpeaking()==false) {
                    time_check();
                    Timer_Stop();
                }
            }
            ...
            switch (event.getAction() & MotionEvent.ACTION_MASK) {
                ...
                case MotionEvent.ACTION_POINTER_UP:  // 두번째 손가락을 화면에서 떼었을 경우
                    ...
                    if (y2drag - y1drag > WHclass.Drag_space) {//손가락 2개를 이용하여 하단으로 드래그 하는경우
                        ...
                        m.check = false;
                        m.getData("http://192.168.0.2:10080/import.php", result);
                        m.page++;
    ...
    public void getText(ArrayList<String> stt){
        for(int i=0; i<stt.size(); i++) {
            char str;
            str = stt.get(i).charAt(i);
            if(str >= '0' && str <= '9') {
                result = stt.get(i);
                break;
            }
