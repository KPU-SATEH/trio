...
public class Menu_word extends FragmentActivity {
    ...
    public void init(){
        ...
        Menu_info.DISPLAY = Menu_info.DISPLAY_WORD;
        m = new Common_menu_display(this);
        m.setBackgroundColor(Color.rgb(22,26,44));

        setContentView(m);
        MainActivity.TTS.speak("Word Practice", TextToSpeech.QUEUE_FLUSH, null);
    }
            ...
            case MotionEvent.ACTION_UP: // 손가락 1개를 화면에서 떨어트렸을 경우
                ...
                if(enter == true) {       //손가락 1개를 떨어트린 x,y좌표 지점에 다시 클릭이 이루어진다면 글자 연습으로 접속
                    if (posx2 < posx1 + WHclass.Touch_space && posx2 > posx1 - WHclass.Touch_space && posy1 < posy2 + WHclass.Touch_space && posy2 > posy2 - WHclass.Touch_space) {
                        WHclass.sel = Menu_info.MENU_MASTER_WORD;
                        Menu_info.MENU_INFO = Menu_info.MENU_MASTER_WORD;
