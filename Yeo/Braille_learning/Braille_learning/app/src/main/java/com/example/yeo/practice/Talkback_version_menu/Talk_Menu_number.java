...
public class Talk_Menu_number extends FragmentActivity {
    MediaPlayer finish;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_common_menu_number);
        finish = MediaPlayer.create(Talk_Menu_number.this, R.raw.basicfinish);

        View container = findViewById(R.id.activity_common_menu_number);
        container.setOnHoverListener(new View.OnHoverListener() {
            @Override
            public boolean onHover(View v, MotionEvent event) {
                switch (event.getAction()) {
                    case MotionEvent.ACTION_HOVER_ENTER: //손가락 1개를 화면에 터치하였을 경우
                        startService(new Intent(Talk_Menu_number.this, Sound_Manager.class));
                    ...
                    case MotionEvent.ACTION_HOVER_EXIT: // 손가락 1개를 화면에서 떨어트렸을 경우
                        ...
                        if(enter == true) {  //손가락 1개를 떨어트린 x,y좌표 지점에 다시 클릭이 이루어진다면 숫자연습으로 접속
                            if (posx2 < posx1 + WHclass.Touch_space && posx2 > posx1 - WHclass.Touch_space && posy1 < posy2 + WHclass.Touch_space && posy2 > posy2 - WHclass.Touch_space) {
                                WHclass.sel=Menu_info.MENU_NUMBER;
                                Menu_info.MENU_INFO = Menu_info.MENU_NUMBER;
                                Intent intent = new Intent(Talk_Menu_number.this, Talk_Braille_short_practice.class);
        ...
        switch(event.getAction() & MotionEvent.ACTION_MASK) {
            case MotionEvent.ACTION_UP:  // 두번째 손가락을 떼었을 경우
                ...
                if(olddrag-newdrag>WHclass.Drag_space) {  //손가락 2개를 이용하여 오른쪽에서 왼쪽으로 드래그할 경우 다음 메뉴로 이동
                    Intent intent = new Intent(this,Talk_Menu_alphabet.class);
                    startActivityForResult(intent,Menu_info.MENU_ALPHABET);
                    Menu_basic_service.menu_page = Menu_info.MENU_ALPHABET;
                    ...
                    Menu_detail_service.menu_page=8;
