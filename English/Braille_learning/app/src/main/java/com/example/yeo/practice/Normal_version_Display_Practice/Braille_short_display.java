...
public class Braille_short_display extends View {
    ...
    public static String textname_alphabet;
    ...
    public void MyView2_init(){
        ...
        switch(WHclass.sel){
            ...
            case 4: //Number Practice
                dot_count = Braille_short_practice.Dot_number.num_dot_count.get(page);
                if (dot_count == 1) { //점자가 6개일떄
                    for (int i = 0; i < 3; i++) {
                        for (int j = 0; j < 2; j++) {
                            text_1[i][j] = dot_number.num_Array.get(page)[i][j];
                            textname_1 = dot_number.num_name.get(page);
                        }
                    }
                } else if (dot_count == 2) { //점자가 12개일때
                    for (int i = 0; i < 3; i++) {
                        for (int j = 0; j < 4; j++) {
                            text_2[i][j] =  dot_number.num_Array.get(page)[i][j];
                            textname_2 = dot_number.num_name.get(page);
                        }
                    }
                } else if (dot_count == 3) { //점자가 18개일때
                    for (int i = 0; i < 3; i++) {
                        for (int j = 0; j < 6; j++) {
                            text_3[i][j] =  dot_number.num_Array.get(page)[i][j];
                            textname_3 = dot_number.num_name.get(page);
                        }
                    }
                }
                break;
                ...
                if (dot_count == 1) { //점자가 6개일떄
                    for (int i = 0; i < 3; i++) {
                        for (int j = 0; j < 2; j++) {
                            ...
                            textname_alphabet = dot_alphabet.alphabet_name2.get(page);
                        }
                    }
                } else if (dot_count == 2) { //점자가 12개일때
                    for (int i = 0; i < 3; i++) {
        ...
        switch(dot_count) {
            case 1:
                if(textname_1.length() <=1)
                    ...
                    canvas.drawText(textname_1, height * (float) 0.15, width * (float) 0.3, paint);
