...
public class Talk_reading_long_practice extends FragmentActivity implements SpeechRecognizeListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(m);
        quiz_reading_service.menu_page = 1;
            ...
            public boolean onHover(View v, MotionEvent event) {
                switch (event.getAction()) {
                    case MotionEvent.ACTION_HOVER_ENTER: //손가락 1개를 화면에 터치하였을 경우
                        startService(new Intent(Talk_reading_long_practice.this, Sound_Manager.class));
                            ...
                            if (m.x < m.notarget7_width[0][0] + m.bigcircle && m.x > m.notarget7_width[0][0] - m.bigcircle && m.y < m.notarget7_height[0][0] + m.bigcircle && m.y > m.notarget7_height[0][0] - m.bigcircle) {
                                ...
                                if (result1 == 0) {
                                    if (m.x < m.target7_width[0][0] + m.bigcircle && m.x > m.target7_width[0][0] - m.bigcircle && m.y < m.target7_height[0][0] + m.bigcircle && m.y > m.target7_height[0][0] - m.bigcircle) {
                                        WHclass.target = true;
                                        startService(new Intent(Talk_reading_long_practice.this, Number.class));
