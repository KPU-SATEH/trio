...
public class Talk_Menu_letter extends FragmentActivity {
    MediaPlayer finish;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        finish = MediaPlayer.create(Talk_Menu_letter.this, R.raw.masterfinish);
        decorView.setSystemUiVisibility( uiOption );
        setContentView(R.layout.activity_common_menu_oneword);
        View container = findViewById(R.id.activity_common_menu_oneword);
        container.setOnHoverListener(new View.OnHoverListener() {
            @Override
            public boolean onHover(View v, MotionEvent event) {
                switch (event.getAction()) {
                    case MotionEvent.ACTION_HOVER_ENTER: //손가락 1개를 화면에 터치하였을 경우
                        startService(new Intent(Talk_Menu_letter.this, Sound_Manager.class));
                    ...
                    case MotionEvent.ACTION_HOVER_EXIT: // 손가락 1개를 화면에서 떨어트렸을 경우
                        ...
                        if(enter == true) {  //손가락 1개를 떨어트린 x,y좌표 지점에 다시 클릭이 이루어진다면 글자퀴즈으로 접속
                            if (posx2 < posx1 + WHclass.Touch_space && posx2 > posx1 - WHclass.Touch_space && posy1 < posy2 + WHclass.Touch_space && posy2 > posy2 - WHclass.Touch_space) {
                                ...
                                Intent intent = new Intent(Talk_Menu_letter.this, Talk_Braille_long_practice.class);
        ...
        switch(event.getAction() & MotionEvent.ACTION_MASK) {
            case MotionEvent.ACTION_UP:  // 두번째 손가락을 떼었을 경우
                ...
                if(olddrag-newdrag>WHclass.Drag_space) { //손가락 2개를 이용하여 오른쪽에서 왼쪽으로 드래그할 경우 다음 메뉴로 이동
                    Intent intent = new Intent(this,Talk_Menu_word.class);
                    ...
                    Menu_detail_service.menu_page=12;
