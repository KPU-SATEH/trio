...
public class writing_short_practice extends FragmentActivity{
    /*
    ...
    */
    ...
    public void Display_Question(){
        ...
        switch(m.question){
            case 1:
                TTS_text ="This is the first question. Enter Braile on the screen"+m.getQuestion();
                break;
            case 2:
                TTS_text ="This is the second question. Enter Braile on the screen"+m.getQuestion();
                break;
            case 3:
                TTS_text ="This is the last question. Enter Braile on the screen"+m.getQuestion();
                break;
        }
    ...
    public void Touch_event(){
        ...
        switch (m.dot_count) {
            case 1: //첫번째 칸의 구분선과 경고음이 발생되는 영역을 지정
                if (m.x > m.width8 + m.bigcircle && m.x < m.width8 + (m.bigcircle * 2) && m.y > m.height1 - (m.bigcircle * 2)) {
                    WHclass.number = 7;
                    WHclass.target = true;
                    startService(new Intent(this, Number.class));
    ...
    public void Grading(){
        ...
        if(result==true) {
            MainActivity.TTS2.speak("That's right!, move on to the next question.",TextToSpeech.QUEUE_FLUSH, null);
        }
        else if(result==false){
            m.quiz_target_init();
            MainActivity.TTS2.speak("That’s a wrong answer.Check out the braille and move on to the next question.", TextToSpeech.QUEUE_FLUSH, null);
            for(int i=0 ; i<3 ; i++) {
                for (int j = 0; j < 6; j++) {
