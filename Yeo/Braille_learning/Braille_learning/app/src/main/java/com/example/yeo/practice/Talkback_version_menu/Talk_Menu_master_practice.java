...
public class Talk_Menu_master_practice extends FragmentActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_common_menu_master_practice);

        View container = findViewById(R.id.activity_common_menu_master_practice);
        container.setOnHoverListener(new View.OnHoverListener() {
            @Override
            public boolean onHover(View v, MotionEvent event) {
                switch (event.getAction()) {
                    case MotionEvent.ACTION_HOVER_ENTER: //손가락 1개를 화면에 터치하였을 경우
                        startService(new Intent(Talk_Menu_master_practice.this, Sound_Manager.class));
                    ...
                    case MotionEvent.ACTION_HOVER_EXIT: // 손가락 1개를 화면에서 떨어트렸을 경우
                        ...
                        if(enter == true) {//손가락 1개를 떨어트린 x,y좌표 지점에 다시 클릭이 이루어진다면 숙련과정으로 접속
                            if (posx2 < posx1 + WHclass.Touch_space && posx2 > posx1 - WHclass.Touch_space && posy1 < posy2 + WHclass.Touch_space && posy2 > posy2 - WHclass.Touch_space) {
                                Intent intent = new Intent(Talk_Menu_master_practice.this, Talk_Menu_letter.class);
                                startActivityForResult(intent, Menu_info.MENU_MASTER_PRACTICE);
                                Menu_master_service.menu_page=Menu_info.MENU_MASTER_LETTER;
                                startService(new Intent(Talk_Menu_master_practice.this, Menu_master_service.class));
                            }
                        }
                        else    enter = true;
                        break;
