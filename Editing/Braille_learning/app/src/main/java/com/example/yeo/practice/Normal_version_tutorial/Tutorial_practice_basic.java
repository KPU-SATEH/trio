...
public class Tutorial_practice_basic extends FragmentActivity {
    ...
    public void first(){
        Runnable updater = new Runnable() {
            ...
            public void run() {
                ...
                if(touch_check>4){
                    n=2;
                    MainActivity.Braille_TTS.TTS_Play("방금 전에 들은것과 같이 ");
                    //lock=true;
                    touch_check=0;
                }
            }
            ...
            public void run() {
                for(int i=0; i<6; i++){
                    if(click_check[i]==true){
                        check_timer++;  // 여기 말이안되는 코드임 그냥 +++++++++해서 6이됨.
                        if(check_timer==6){
                            n=3;
                            MainActivity.Braille_TTS.TTS_Play("다음으로 넘어가셈");
                            check_timer=0;
                        }
                    }else
                        break;
        ...
        Runnable updater = new Runnable() {
            ...
            public void run() {
                if(m.page ==1) {
                    ...
                    if (touch_check > 6) {
                        if (Speak_check == true) {
                            MainActivity.Braille_TTS.TTS_Play("두칸으로되잇음 구분선 터치하셈");
                            Speak_check = false;
                        }
                    }
                }
                for(int i=7; i<12; i++){
                    if(click_check[i]==true){
                        check_timer++;
                        if(check_timer==2){
                            MainActivity.Braille_TTS.TTS_Play("굿뜨굿뜨굿뜨");
                            n=4;
                        }
                    }else
                        break;
