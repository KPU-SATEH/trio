...
import static com.example.yeo.practice.Normal_version_Display_Practice.writing_short_display.Dot_quiz_alphabet;
import static com.example.yeo.practice.Normal_version_Display_Practice.writing_short_display.Dot_quiz_number;
import static com.example.yeo.practice.Normal_version_Display_Practice.writing_short_display.Dot_quiz_sentence;
    ...
    public void quiz_view_init(){
        ...
        */
        switch(Menu_info.MENU_QUIZ_INFO){
            case 0: //
                max = Dot_quiz_alphabet.alphabet_count;
                random = new Random();
                ...
                break;
            case 1: //
                max = Dot_quiz_number.num_count;
                random = new Random();
                page = random.nextInt(max) + min;
                dot_count = Dot_quiz_number.num_dot_count.get(page);

                if(dot_count==1) { //점자가 6개일떄
                    for (int i = 0; i < 3; i++) {
                        for (int j = 0; j < 2; j++) {
                            text_1[i][j] = Dot_quiz_number.num_Array.get(page)[i][j];
                            textname_1 = Dot_quiz_number.num_name.get(page);
                        }
                    }
                }
                else if(dot_count==2) { //점자가 12개일때
                    for (int i = 0; i < 3; i++) {
                        for (int j = 0; j < 4; j++) {
                            text_2[i][j] = Dot_quiz_number.num_Array.get(page)[i][j];
                            textname_2 = Dot_quiz_number.num_name.get(page);
                    ...
                    for (int i = 0; i < 3; i++) {
                        for (int j = 0; j < 6; j++) {
                            text_3[i][j] = Dot_quiz_number.num_Array.get(page)[i][j];
                            textname_3 = Dot_quiz_number.num_name.get(page);
                ...
                break;
            case 2: //
                max = Dot_quiz_sentence.sen_count;
                random = new Random();
                page = random.nextInt(max) + min;
