 ...
 */
/*
 * 초성 연습에서 사용되는 점자의 배열정보 및 글자정보를 관리하는 클래스
 */
public class Trans_dot_initial {

    static public int Initial_count = 19; //초성 연습의 갯수
    ...
    public static String name [] ={ "ㄱ","ㄴ","ㄷ","ㄹ","ㅁ","ㅂ","ㅅ","ㅈ","ㅊ","ㅋ","ㅌ","ㅍ","ㅎ","된소리표","ㄲ","ㄸ","ㅃ","ㅆ","ㅉ"};
    ...
    public int Dot_count_search(int index){
        int count=0;
        count = Initial_dot_count.get(index);
        return count;
    }
    ...
    public int Name_search(String name){
        int index=0;
        boolean check = false;
        for(int i=0; i<Initial_name.size() ; i++){
            if(name.equals(Initial_name.get(i))==true){
                index=i;
                check = true;
                break;
            }
    ...
    public int[][] Matrix_search(int index, int[][] matrix){
        int count = Initial_dot_count.get(index);
        count = count *2;
        for(int i=0 ; i<3 ; i++){
            for(int j=0 ; j<count ; j++){
                matrix[i][j]=Initial_Array.get(index)[i][j];
            }
        }
