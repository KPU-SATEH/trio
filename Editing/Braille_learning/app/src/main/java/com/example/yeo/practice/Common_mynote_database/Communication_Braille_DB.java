...
/**
 * Created by 여명 on 2017-02-21.
 */

public class Communication_Braille_DB extends SQLiteOpenHelper {
    ...
    public Communication_DB_manager communication_db_manager;


    public Communication_Braille_DB(Context context, String name, SQLiteDatabase.CursorFactory factory, int version) {
        super(context, name, factory, version);
        communication_db_manager = new Communication_DB_manager();
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE BRAILLE3( _id INTEGER PRIMARY KEY AUTOINCREMENT, " + "count INTEGER," + "name STRING," + "matrix1 TEXT," +
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) { //버전이 올라갔을 때 기존의 테이블을 삭제하고 다시 만듦
        db.execSQL("DROP TABLE IF EXISTS BRAILLE3");
        onCreate(db);
    }
    ...
    public String insert(int count, String name, String matrix1, String matrix2, String matrix3, int reference, int reference_index){ //데이터베이스에 데이터 저장
        ...
        Cursor cursor = db.rawQuery("SELECT * FROM BRAILLE3", null);
        while(cursor.moveToNext()){
            communication_db_manager.datacount++;
        }

        if(communication_db_manager.datacount<communication_db_manager.MAXSIZE) {
            db.execSQL("INSERT INTO BRAILLE3 VALUES(null,'" + count + "', '" + name + "', '" + matrix1 + "', '" + matrix2 + "', '" + matrix3 + "', '" + reference + "', '" + reference_index + "');");
            communication_db_manager.MyNote_down = false;
            result = "성공";
        }
        else{
            result = "실패";
    ...
    public void delete(int number){ //데이터베이스 삭제
        SQLiteDatabase db = getWritableDatabase();
        String sql = "DELETE FROM BRAILLE3 WHERE _id="+number;
        db.execSQL(sql);


        if(communication_db_manager.My_Note_page>0) communication_db_manager.My_Note_page--;
    ...
    public String getResult(){  //데이터베이스로부터 값을 읽어옴
        ...
        Cursor cursor  = db.rawQuery("SELECT * FROM BRAILLE3",null);
        communication_db_manager.DB_init(); //현재까지 읽어들인 데이터베이스 초기화
        ...
        while(cursor.moveToNext()){
            ...
            communication_db_manager.DB_All_insert(id,count, name, matrix1, matrix2, matrix3, reference, reference_index); //데이터베이스에 저장
        }
