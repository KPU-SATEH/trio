...
public class Talk_Menu_quiz_alphabet extends FragmentActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //quiz_reading_service.finish_n = 4;

        decorView.setSystemUiVisibility( uiOption );
        setContentView(R.layout.activity_common_menu_quiz_alpabet);
        View container = findViewById(R.id.activity_common_menu_quiz_alphabet);
        container.setOnHoverListener(new View.OnHoverListener() {
            @Override
            public boolean onHover(View v, MotionEvent event) {
                switch (event.getAction()) {
                    case MotionEvent.ACTION_HOVER_ENTER: //손가락 1개를 화면에 터치하였을 경우
                        startService(new Intent(Talk_Menu_quiz_alphabet.this, Sound_Manager.class));
                    ...
                    case MotionEvent.ACTION_HOVER_EXIT: // 손가락 1개를 화면에서 떨어트렸을 경우
                        ...
                        if(enter == true) {  //손가락 1개를 떨어트린 x,y좌표 지점에 다시 클릭이 이루어진다면 알파벳 퀴즈로 접속
                            if (posx2 < posx1 + WHclass.Touch_space && posx2 > posx1 - WHclass.Touch_space && posy1 < posy2 + WHclass.Touch_space && posy2 > posy2 - WHclass.Touch_space) {
                                Menu_info.MENU_QUIZ_INFO = Menu_info.MENU_QUIZ_ALPHABET;
                                manual.choice=5;
                                score.sel =5;
                                Intent intent = new Intent(Talk_Menu_quiz_alphabet.this, Talk_Menu_quiz_reading.class);
        ...
        switch(event.getAction() & MotionEvent.ACTION_MASK) {
            case MotionEvent.ACTION_UP:  // 두번째 손가락을 떼었을 경우
                ...
                if(olddrag-newdrag>WHclass.Drag_space) {  //손가락 2개를 이용하여 오른쪽에서 왼쪽으로 드래그할 경우 다음 메뉴로 이동
                    Intent intent = new Intent(this,Talk_Menu_quiz_sentence.class);
                    startActivityForResult(intent,Menu_info.MENU_QUIZ_SENTENS);
                    Menu_quiz_service.menu_page= Menu_info.MENU_QUIZ_SENTENS;
                    ...
                    Intent intent = new Intent(this,Talk_Menu_quiz_number.class);
                    startActivityForResult(intent,Menu_info.MENU_QUIZ_NUMBER);
                    Menu_quiz_service.menu_page=Menu_info.MENU_QUIZ_NUMBER;
                    startService(new Intent(this, Menu_quiz_service.class));
                    ...
                    finish();
                      ...
                      Menu_detail_service.menu_page=18;
