...
public class Braille_translation {
    ...
    Trans_dot_final Trans_dot_final; //종성 점자 저장 클래스
    Trans_dot_abbreviation Trans_dot_abbreviation; //약자 및 약어 점자 저장 클래스
    Trans_dot_num transDot_number; //숫자 점자 저장 클래스
    Trans_dot_alphabet Trans_dot_alphabet;
    ...
    public Braille_translation(){
        ...
        Trans_dot_abbreviation = new Trans_dot_abbreviation();
    ...
    public void Alphabet_Braille_search(String Alphabet_text){
        ...
        switch(type){
            case 4: // 알파벳 소문자
                count= Trans_dot_alphabet.Name_search(Kor_text);                              //알파벳 탐색 시작
    ...
    public void Number_Braille_search(String Number_text){
        ...
        switch(type){
            case 3:
                count= transDot_number.Name_search(Kor_text);                              //숫자 탐색 시작
    ...
    public void Hangel_Braille_search(String Hangel_text){
        ...
        switch(type) {
            case 0:
                ...
                count = Trans_dot_abbreviation.Name_search(Kor_text,abbrevion_check); // 약자 및 약어에서 탐색 시작
                ...
                if(count == -2){ // 단어 길이가 긴 약자 및 약어 중 글자가 매칭되는게 있다면
                    ...
                    if(abbrevion_check!=0) {
                        ...
                        count = Trans_dot_abbreviation.Name_search(initial_vowel, abbrevion_check);
                        if (count != -1) {                                                //-1이 아니면 약자 및 약어 탐색 성공
                            Abbreviation_search(count);
                            if (finalcode != 0) {
                                count = Trans_dot_final.Name_search(jongsung[finalcode]);          //종성 서치 시작
                            ...
                            count = Trans_dot_abbreviation.Name_search(vowel_final, abbrevion_check);
                            if (count != -1) {
                                int count2 = Trans_dot_initial.Name_search(chosung[initcode]);         //초성 서치 시작
                                if (count2 != -1)
                                    Initial_search(count2);
                                Abbreviation_search(count);
                            }
                            else {                                         // -1일 경우 약자 및 약어 탐색 실패, 초성과 모음 점자 서치 시작
                                count = Trans_dot_initial.Name_search(chosung[initcode]);         //초성 서치 시작
                                if (count != -1)
                                    Initial_search(count);

                                count = Trans_dot_vowel.Name_search(moum[vowelcode]);             //모음 서치 시작
                                if (count != -1)
                                    Vowel_search(count);
                    ...
                    if(abbrevion_check!=0){
                        if(abbreviation_search_success==true) { // 단어 길이가 긴 약자 및 약어 탐색에 성공했으면
                            int name_length=0;
                            name_length= Trans_dot_abbreviation.get_name(count).length();
                ...
                break;
            case 1:
                count= Trans_dot_initial.Name_search(Kor_text);                              //초성 탐색 시작
                if (count != -1)                                                        //-1이 아니면 초성탐색 성공
                    Initial_search(count);
                break;
            case 2:
                count = Trans_dot_vowel.Name_search(Kor_text);                      //모음 탐색 시작
    ...
    public void Alphabet_search(int count){
        if(alphabet_check==false){ // 숫자가 처음 나타난 것이라면
            //숫자 앞에 수표를 붙임
            int roma=0;
            dot_count = Trans_dot_alphabet.Dot_count_search(roma);                       // 발견한 점자가 몇칸인지 파악
            int matrix_temp[][] = new int[3][dot_count * 2];                        //칸수에 맞게 행렬 선언
            matrix_temp = Trans_dot_alphabet.Matrix_search(roma, matrix_temp);          //임시 행렬에 탐색한 행렬 삽입
            ...
            if(type==5){
                int fortis=1;
                dot_count = Trans_dot_alphabet.Dot_count_search(fortis);                       // 발견한 점자가 몇칸인지 파악
                int matrix_temp2[][] = new int[3][dot_count * 2];                        //칸수에 맞게 행렬 선언
                matrix_temp2 = Trans_dot_alphabet.Matrix_search(fortis, matrix_temp2);          //임시 행렬에 탐색한 행렬 삽입
                matrix_count += dot_count * 2;                                       //삽입한 점자 칸 수만큼 행렬값 증가
    ...
    public void Abbreviation_search(int count){
        dot_count = Trans_dot_abbreviation.Dot_count_search(count);                       // 발견한 점자가 몇칸인지 파악
        int matrix_temp[][] = new int[3][dot_count * 2];                        //칸수에 맞게 행렬 선언
        matrix_temp = Trans_dot_abbreviation.Matrix_search(count, matrix_temp);          //임시 행렬에 탐색한 행렬 삽입
        matrix_count += dot_count * 2;                                       //삽입한 점자 칸 수만큼 행렬값 증가
        matrix_print(matrix_temp);                                                  //발견한 점자 행렬을 출력
    }
    ...
    public void Initial_search(int count){
        dot_count = Trans_dot_initial.Dot_count_search(count);
        int matrix_temp[][] = new int[3][dot_count * 2];                    //칸수에 맞게 행렬 선언
        matrix_temp = Trans_dot_initial.Matrix_search(count, matrix_temp);            //임시 행렬에 탐색한 행렬 삽입
        matrix_count += dot_count * 2;                                      //삽입한 점자 칸 수만큼 행렬값 증가
        matrix_print(matrix_temp);                                          //발견한 점자 행렬을 출력
    }
    ...
    public void Vowel_search(int count){
        dot_count = Trans_dot_vowel.Dot_count_search(count);
        int matrix_temp2[][] = new int[3][dot_count * 2];                   //칸수에 맞게 행렬 선언
        matrix_temp2 = Trans_dot_vowel.Matrix_search(count, matrix_temp2);        //임시 행렬에 탐색한 행렬 삽입
        matrix_count += dot_count * 2;                                  //삽입한 점자 칸 수만큼 행렬값 증가
    ...
    public void Final_search(int count){
        dot_count = Trans_dot_final.Dot_count_search(count);
        int matrix_temp[][] = new int[3][dot_count * 2];                    //칸수에 맞게 행렬 선언
        matrix_temp = Trans_dot_final.Matrix_search(count, matrix_temp);          //임시 행렬에 탐색한 행렬 삽입
        matrix_count += dot_count * 2;                      //삽입한 점자 칸 수만큼 행렬값 증가
        matrix_print(matrix_temp);                          //발견한 점자 행렬을 출력
    }
